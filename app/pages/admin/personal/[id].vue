<template>
  <div class="m-6 ml-12">
    ...
    <div v-else-if="state.status === 'error'">Ocurrio un error inesperado</div>
    <div v-else class="grid grid-cols-2 mt-4">
      <div class="flex flex-col gap-4">
        <div class="flex flex-row gap-4">
          <h1 class="text-3xl font-semibold">
            {{
              `${state.data.employee.firstName} ${state.data.employee.lastName}`
            }}
          </h1>
        ...
        <div class="flex flex-row gap-6">
          <div class="flex flex-row h-6 gap-4">
            ...
            <Tag
              :severity="
                state.data.employee.igssPercentage ? 'success' : 'danger'
              "
            >
              {{
                state.data.employee.igssPercentage
                  ? `${state.data.employee.igssPercentage}%`
                  : "No Aplica"
              }}</Tag
            >
          ...
          <div class="flex flex-row h-6 gap-4">
            ...
            <Tag
              :severity="
                state.data.employee.irtraPercentage ? 'success' : 'danger'
              "
            >
              {{
                state.data.employee.irtraPercentage
                  ? `${state.data.employee.irtraPercentage}%`
                  : "No Aplica"
              }}
            </Tag>
        ...
        <div>
          ...
          <template v-if="availableCashRegisters.status !== 'error'">
            ...
            <Select
              ...
              :options="availableCashRegisters.data"
              optionLabel="code"
              placeholder="Selecciona una caja registradora"
              class="w-full md:w-56"
            />
            <Button
              class="ml-2"
              label="Asignar"
              @click="submitChangeCashRegister"
            />
          </template>
        </div>
      </div>
              ...
              <template #body="{ data }">
                <div class="flex items-center gap-2">
                  <p class="text-lg font-medium">
                    {{
                      data.historyType.type === "Aumento Salarial" ||
                      data.historyType.type === "Disminucion Salarial"
                        ? `Salario modificado a Q.${data.commentary}`
                        : data.commentary
                    }}
                  </p>
                </div>
...
import {
  changeCashRegisterToEmployee,
  getAllCashRegisters,
  getCashRegisterByEmployeeId,
  updateCashRegister,
  type CashRegister,
  type UpdateCashRegister,
} from "~/lib/api/cash_register/cashRegister";
  ...
  mutation: async (updateData: UpdateCashRegister) => {
    if (cashRegister.value.status === "error") {
      ...
      return updateCashRegister(
        selectedCashRegister.value?.id ?? "",
        updateData
      );
    } else {
      //Si ya tenia asignada una se tiene que desasignar primero esa y luego se asigna la nueva
      if (!selectedCashRegister.value?.id) {
        toast.warning("Selecciona una caja registradora");
        return cashRegister.value.data;
      }
      if (!updateData.employeeId) {
        toast.warning("Selecciona un empleado");
        return cashRegister.value.data;
      }
      const newCashRegister = await changeCashRegisterToEmployee(
        updateData.employeeId,
        selectedCashRegister.value.id
      );
      selectedCashRegister.value = newCashRegister;
...
const filteredVacations = computed(() => {
  if (
    selectedYear.value &&
    state.value.data?.vacations &&
    state.value.data.vacations[selectedYear.value]
  ) {
...
watch(
  ...
  (data) => {
    if (data.status === "success") {
      selectedCashRegister.value = data.data;
    }
  },
  { immediate: true }
);
